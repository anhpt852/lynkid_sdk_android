...
import vn.linkid.sdk.databinding.ItemAddressPickerBinding
import vn.linkid.sdk.models.address.Address

class AddressPickerAdapter(private val listAddress: List<Address>) :
    RecyclerView.Adapter<AddressPickerAdapter.AddressPickerViewHolder>() {
    var onItemClick: ((Address) -> Unit)? = null
    var filteredAddress: List<Address> = listAddress.toList()
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AddressPickerViewHolder {
        val binding =
            ItemAddressPickerBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    inner class AddressPickerViewHolder(private val binding: ItemAddressPickerBinding) :
