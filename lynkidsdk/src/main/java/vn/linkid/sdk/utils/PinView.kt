...
class PinView @JvmOverloads constructor(
    ...
    private fun setupAnimator() {
        ...
        if (mDefaultAddAnimator is ValueAnimator) {
            (mDefaultAddAnimator as ValueAnimator).setDuration(150)
            (mDefaultAddAnimator as ValueAnimator).interpolator = DecelerateInterpolator()
            (mDefaultAddAnimator as ValueAnimator).addUpdateListener { animation ->
    ...
    private fun drawAnchorLine(canvas: Canvas) {
        ...
        mPath.lineTo(
            cx,
            (mItemBorderRect.top + abs(mItemBorderRect.height().toDouble())).toFloat()
        )
        canvas.drawPath(mPath, mPaint)
...
//            }
    ...
    private class DefaultActionModeCallback : ActionMode.Callback {
        override fun onCreateActionMode(mode: ActionMode, menu: Menu): Boolean {
            return false
        }
